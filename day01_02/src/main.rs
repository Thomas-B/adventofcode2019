...
fn calculate_fuel_requirement(module_weight: i32) -> i32 {
	let result = (module_weight / 3) - 2;

	if result <= 0 {
		return 0;
	}

	return result + calculate_fuel_requirement(result);
}
...
mod tests {
	...
	fn simple() {
		...
		assert_eq!(calculate_fuel_requirement(1969), 966);
		assert_eq!(calculate_fuel_requirement(100756), 50346);
	}
}
